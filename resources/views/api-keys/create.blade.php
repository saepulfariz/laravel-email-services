...
@section('title', 'Generate API Key - Email Services')

@section('content')
    <div class="mb-8">
        <a href="{{ route('api-keys.index') }}" class="text-steel no-underline text-sm font-medium">&larr; Back to API Keys</a>
        <h1 class="mt-4">Generate API Key</h1>
        <p class="subtitle mb-0">Generate a new API key and configure its service access.</p>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="md:col-span-1">
                <div class="card h-full">
                    <h3 class="mt-0 text-lg">Key Details</h3>
                    <p class="text-steel text-sm mb-6">Basic information about this key.</p>

                    <div class="input-group">
                        <label>Key Name <span class="text-[#d45656]">*</span></label>
                        <input type="text" name="name" required value="{{ old('name') }}" class="text-input" placeholder="e.g. Production Server Key">
                    </div>

                    <div class="input-group">
                        <label>Expiration Date <span class="text-steel font-normal">(Optional)</span></label>
                        <input type="date" name="expires_at" value="{{ old('expires_at') }}" class="text-input">
                    </div>
                </div>
            </div>
            ...
            <div class="md:col-span-2">
                <div class="card h-full">
                    <h3 class="mt-0 text-lg">Service Access & Rate Limits</h3>
                    <p class="text-steel text-sm mb-6">Select which services this key can access and configure limits.</p>

                    @if($services->isEmpty())
                        <div class="p-4 bg-[#fff8e1] text-[#f57f17] border border-[#ffe082] rounded-lg">
                            No active services available. Please create a service first.
                        </div>
                    @endif
                            ...
                            <div class="border border-hairline rounded-xl overflow-hidden">
                                ...
                                <div class="bg-surface px-4 py-3 border-b border-hairline flex items-center gap-3">
                                    ...
                                    <label for="service_{{ $service->id }}" class="font-semibold text-ink cursor-pointer select-none mb-0">
                                        {{ $service->name }}
                                        <span class="block text-xs font-normal text-steel">{{ $service->description ?? 'No description' }}</span>
                                    </label>
                                </div>

                                <!-- Settings block (Hidden by default) -->
                                <div id="settings_{{ $service->id }}" class="service-settings p-4 space-y-4 bg-white {{ is_array(old('services')) && in_array($service->id, old('services')) ? '' : 'hidden' }}">
                                    
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div class="input-group mb-0">
                                            <label class="text-xs">Allowed IPs (Comma separated)</label>
                                            <input type="text" name="service_settings[{{ $service->id }}][allowed_ips]" placeholder="e.g. 192.168.1.1, 10.0.0.1"
                                                class="text-input font-mono text-sm py-1.5"
                                                value="{{ old('service_settings.'.$service->id.'.allowed_ips') }}">
                                        </div>
                                        <div class="input-group mb-0">
                                            <label class="text-xs">Allowed Domains (Comma separated)</label>
                                            <input type="text" name="service_settings[{{ $service->id }}][allowed_domains]" placeholder="e.g. app.example.com"
                                                class="text-input font-mono text-sm py-1.5"
                                                value="{{ old('service_settings.'.$service->id.'.allowed_domains') }}">
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div id="rate_limits_container_{{ $service->id }}" class="space-y-2">
                                            <!-- Dynamically added rate limits go here -->
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        checkboxes.forEach(cb => {
            cb.addEventListener('change', function() {
                ...
                if (this.checked) {
                    ...
                    // Add default rate limit if empty
                    const container = document.getElementById('rate_limits_container_' + this.value);
                    if(container.children.length === 0) {
                        addRateLimitRow(this.value);
                    }
                } else {
